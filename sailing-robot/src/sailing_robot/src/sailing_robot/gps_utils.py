...
import struct
import sys
from pynmea2 import NMEASentence    # 使用 PyNMEA2 解析 NMEA 0183 协议 
...
class UbxNmeaParser(object):
    """Parse a mixed stream of UBX and NMEA messages 解析一个UBX和NMEA消息的混合流
    ...
    This parser reads the two types of messages, along with any other chunks（块）
    (e.g. if we start reading half-way through a message). 
    
    Use .feed(data) to supply newly read data, and .get_msgs() to get an iterator of bytes and
...
def test_stream_parser(filename):   # parser:解析器
    import random
    parser = UbxNmeaParser()        # 定义了一个解析器对象
    with open(filename, 'rb') as f: # 使用'rb'按照二进制位进行读取文件
        while True:
...
if __name__ == '__main__':
    test_stream_parser(sys.argv[1]) # 在命令行中通过命令行的参数传入文件名
